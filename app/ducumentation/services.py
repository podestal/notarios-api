...
class EscrituraPublicaDocumentService:
    """
    Django service to generate escritura publica documents based on the PHP legacy script
    """
    
    def __init__(self):
        self.letras = NumberToLetterConverter()
    
    def generate_escritura_publica_document(self, template_id: int, num_kardex: str, idtipoacto: str, idtipkar: str, action: str = 'generate', mode: str = "download") -> HttpResponse:
        """
    ...
    def _get_data_documento(self, raw_data: dict) -> Dict[str, str]:
        ...
        fecha_escritura = raw_data.get('fecha_escritura')
        numero_minuta = raw_data.get('numero_minuta') or ''
        sede_colegio = str(raw_data.get('sede_colegio', '')).strip()
        ...
        # Handle sede colegio logic
        sede = ''
        if sede_colegio == 'PUNO':
            sede = 'CAP.'
        elif sede_colegio == 'AREQUIPA':
            sede = 'CAA.'
        ...
        return {
            ...
            'F': fecha_acta,
            'DESCRIPCION_SELLO': f"{raw_data.get('abogado', '')} {sede} {raw_data.get('matricula', '')}",
        }
    ...
    def _get_data_contratantes(self, raw_data: dict) -> Dict[str, list]:
        ...
        departamentos_empresa = split_if_not_none(raw_data.get('departamento_empresa'))
        condiciones_empresa = split_if_not_none(raw_data.get('condicion_empresa'))
        oficinas_registrales = raw_data.get('oficina_registral', '').split(',,') if raw_data.get('oficina_registral') else []
        ...
        for k, condicion in enumerate(condiciones):
            ...
            num_doc_empresa = numeros_documento_empresa[k] if k < len(numeros_documento_empresa) else ''
            ...
            departamento_empresa = departamentos_empresa[k] if k < len(departamentos_empresa) else ''
            ...
            # TRANSFERENTES (VENDEDOR, PODERDANTE, etc.)
            if condicion in ['VENDEDOR', 'PODERDANTE', 'OTORGANTE', 'REPRESENTANTE', 'ANTICIPANTE', 'ADJUDICANTE', 'DONANTE', 'USUFRUCTUANTE', 'TRANSFERENTE', 'TITULAR', 'MUTUANTE', 'PROPIETARIO', 'DEUDOR', 'ASOCIANTE', 'TRANSFERENTE / PROPIETARIO (VENDEDOR)']:
                transferentes.append(contractor_data)
                sexo_transferentes.append(sexo)
                
            # ADQUIRIENTES (COMPRADOR, APODERADO, etc.)
            elif condicion in ['COMPRADOR', 'APODERADO', 'ANTICIPADO', 'ADJUDICATARIO', 'DONATARIO', 'USUFRUCTUARIO', 'TESTIGO A RUEGO', 'ADQUIRIENTE', 'ACREEDOR', 'OTORGADO', 'MUTUATARIO', 'BENEFICIARIA', 'ASOCIADO', 'ADQUIRENTE / BENEFICIARIO (COMPRADOR)']:
    ...
    def _process_contratantes_data(self, contratantes_data: Dict[str, list], raw_data: dict = None) -> Dict[str, str]:
        ...
        # Process transferentes
        transferentes = contratantes_data['transferentes']
        adquirientes = contratantes_data['adquirientes']
        
        # Check for married couples in transferentes
        casados_transferentes = self._check_married_couples(transferentes)
        casados_adquirientes = self._check_married_couples(adquirientes)
        
        # Process transferentes (P)
        contador_vendedor = 1
        for k, transferente in enumerate(transferentes):
            agregado = '' if contador_vendedor == 1 else f'_{contador_vendedor}'
            ...
            if transferente['condicionEmpresa'] in ['EMPRESA EN CONSTITUCION', 'ASOCIACION EN CONSTITUCION']:
                empresa_keys = {
                    'nombre': 'NOMBRE_EMPRESA_2',
                    'inscripcion': 'INS_EMPRESA_2',
                    'ruc': 'RUC_2',
                    'domicilio': 'DOMICILIO_EMPRESA_2',
                    'condicion': 'CONDICION_EMPRESA_2'
                }
            else:
                empresa_keys = {
                    'nombre': 'NOMBRE_EMPRESA_1',
                    'inscripcion': 'INS_EMPRESA_1',
                    'ruc': 'RUC_1',
                    'domicilio': 'DOMICILIO_EMPRESA_1',
            ...
            final_data.update({
                ...
                empresa_keys['domicilio']: f"CON DOMICILIO EN {transferente['direccionEmpresa']} DEL DISTRITO DE {transferente['distritoEmpresa']} PROVINCIA DE {transferente['provinciaEmpresa']} Y DEPARTAMENTO DE {transferente['departamentoEmpresa']}",
                empresa_keys['condicion']: transferente['condicionEmpresa'],
            })
            contador_vendedor += 1
        
        # Process adquirientes (C)
        contador_adquiriente = 1
        for k, adquiriente in enumerate(adquirientes):
            agregado = '' if contador_adquiriente == 1 else f'_{contador_adquiriente}'
            ...
            if contador_adquiriente > 1:
                domicilio = f"CON DOMICILIO EN {adquiriente['direccion']} DEL DISTRITO DE {adquiriente['distrito']} PROVINCIA DE {adquiriente['provincia']} Y DEPARTAMENTO DE {adquiriente['departamento']}"
            else:
            ...
            final_data.update({
                ...
                'DOMICILIO_EMPRESA_2': f"CON DOMICILIO EN {adquiriente['direccionEmpresa']} DEL DISTRITO DE {adquiriente['distritoEmpresa']} PROVINCIA DE {adquiriente['provinciaEmpresa']} Y DEPARTAMENTO DE {adquiriente['departamentoEmpresa']}",
            })
            contador_adquiriente += 1
        
        # Set AMBOS flags based on married couple status
        final_data['P_AMBOS'] = ' AMBOS ' if casados_transferentes else ' '
        final_data['C_AMBOS'] = ' AMBOS ' if casados_adquirientes else ' '
        
        # Add empty placeholders for unused slots
        self._add_empty_placeholders(final_data, len(transferentes), len(adquirientes))
        
        return final_data

    def _check_married_couples(self, contractors: list) -> bool:
        """Check if there are married couples among contractors"""
        if len(contractors) < 2:
            return False
            
        clientes = [c['idCliente'] for c in contractors]
        conyuges = [c['idConyuge'] for c in contractors]
        
        for cliente in clientes:
            if cliente in conyuges:
                return True
        
        for conyuge in conyuges:
            if conyuge != 'NO' and conyuge in clientes:
                return True
    ...
    def _get_articulos_contratantes(self, contratantes_data: Dict[str, list]) -> Dict[str, str]:
        """
        Get grammatical articles for contractors based on complex PHP logic
        """
        transferentes = contratantes_data['transferentes']
        adquirientes = contratantes_data['adquirientes']
        sexo_transferentes = contratantes_data['sexoTransferentes']
        sexo_adquirientes = contratantes_data['sexoAdquirientes']
        
        # Process transferentes articles
        articulos_p = self._get_articulos_singular_plural(transferentes, 'P', sexo_transferentes)
        
        # Process adquirientes articles
        articulos_c = self._get_articulos_singular_plural(adquirientes, 'C', sexo_adquirientes)
        
        # Combine all articles
        articulos = {}
        articulos.update(articulos_p)
        articulos.update(articulos_c)
    ...
    def _get_articulos_singular_plural(self, contractors: list, prefix: str, sexo_group: str) -> Dict[str, str]:
        """
        Generate grammatical articles for singular/plural contractors
        """
        cantidad = len(contractors)
        articulos = {}
        
        if cantidad >= 2:
            # PLURAL
            for contractor in contractors:
                condicion = contractor['condiciones']
                
                # Determine quality based on condition and gender
                if condicion == 'COMPRADOR':
                    calidad = 'COMPRADORAS' if sexo_group == 'MUJERES' else 'COMPRADORES'
                elif condicion == 'VENDEDOR':
                    calidad = 'VENDEDORAS' if sexo_group == 'MUJERES' else 'VENDEDORES'
                elif condicion == 'DONANTE':
                    calidad = 'DONANTES'
                elif condicion == 'DONATARIO':
                    calidad = 'DONATARIAS' if sexo_group == 'MUJERES' else 'DONATARIOS'
                elif condicion == 'PODERDANTE':
                    calidad = 'PODERDANTES'
                elif condicion == 'APODERADO':
                    calidad = 'APODERADAS' if sexo_group == 'MUJERES' else 'APODERADOS'
                elif condicion == 'OTORGANTE':
                    calidad = 'OTORGANTES'
                else:
                    calidad = condicion
                
                articulos[f'EL_{prefix}'] = 'LAS' if sexo_group == 'MUJERES' else 'LOS'
                articulos[f'CALIDAD_{prefix}'] = calidad
                articulos[f'{prefix}_CALIDAD'] = calidad
                break
            
            articulos[f'INICIO_{prefix}'] = ' SEÑORAS' if sexo_group == 'MUJERES' else ' SEÑORES'
            articulos[f'ES_{prefix}'] = 'ES'
            articulos[f'S_{prefix}'] = 'S'
            articulos[f'ES_SON_{prefix}'] = 'SON'
            articulos[f'Y_CON_{prefix}'] = 'Y'
            articulos[f'N_{prefix}'] = 'N'
            articulos[f'Y_{prefix}'] = 'Y'
            articulos[f'L_{prefix}'] = 'L'
            articulos[f'O_A_{prefix}'] = 'OS'
            articulos[f'O_ERON_{prefix}'] = 'ERON'
            articulos[f'O_ARON_{prefix}'] = 'ARON'
            articulos[f'{prefix}_FIRMA'] = 'FIRMAN EN'
            
        elif cantidad == 1:
            # SINGULAR
            contractor = contractors[0]
            condicion = contractor['condiciones']
            sexo = contractor['sexo']
            ...
            if sexo == 'F':
                ...
                articulos[f'EL_{prefix}'] = 'LA'
                articulos[f'INICIO_{prefix}'] = ' SEÑORA'
            else:
                articulos[f'EL_{prefix}'] = 'EL'
                articulos[f'INICIO_{prefix}'] = ' SEÑOR'
                calidad = condicion
            
            articulos[f'CALIDAD_{prefix}'] = calidad
            articulos[f'{prefix}_CALIDAD'] = calidad
            articulos[f'ES_{prefix}'] = ''
            articulos[f'S_{prefix}'] = ''
            articulos[f'ES_SON_{prefix}'] = 'ES'
            articulos[f'Y_CON_{prefix}'] = ''
            articulos[f'N_{prefix}'] = ''
            articulos[f'Y_{prefix}'] = ''
            articulos[f'L_{prefix}'] = ''
            articulos[f'O_A_{prefix}'] = 'O'
            articulos[f'O_ERON_{prefix}'] = 'O'
            articulos[f'O_ARON_{prefix}'] = 'O'
            articulos[f'{prefix}_FIRMA'] = 'FIRMA EN'
            articulos[f'{prefix}_AMBOS'] = ' '
    ...
    def create_documento_in_r2(self, doc, kardex):
        ...
        try:
            ...
            print(f"Error uploading escritura publica document to R2: {e}")
            return False
