...
class GarantiasDocumentService:
    """
    Service to generate Garantias Mobiliarias documents
    Mirrors: PHP garantias_mobiliarias() function
    ...
    def generate_garantias_document(self, template_id, kardex, action, mode):
        """
        Main entry point for generating Garantias documents
        Mirrors: PHP garantias_mobiliarias() function
        
        FLOW: (Same as Escrituras and No Contenciosos)
        ...
        try:
            ...
            print(f"DEBUG: Falling back to PlaceholderProcessor for Garantias")
    ...
    def _update_existing_document(self, kardex, mode):
        """
        Update existing Garantias document with escrituracion data
        ...
        raw_data = self._consulta_garantias(kardex, "actualizar", None)
        data_escrituracion = self.formatter.format_escrituracion_data(raw_data)
    ...
    def _consulta_garantias(self, num_kardex, action, template_id):
        """
        SQL Query for Garantias documents
        Mirrors: PHP consulta_transferencia() in garantias_mobiliarias.php
        
        NOTE: The query structure is IDENTICAL to Escrituras and No Contenciosos,
        ...
        # Same query as Escrituras and No Contenciosos (all use same data structure)
        query = """
...
class GarantiasReportService:
    ...
    def generate_excel_report(self, desde, hasta):
        ...
        try:
            ...
            ws[f"A{row}"].font = header_font
            ws[f"A{row}"].border = no_border
            ws[f"B{row}"] = f': {notary_info["departamento"]}'
            ws[f"B{row}"].font = data_font
            ws[f"B{row}"].border = no_border
            ws[f"C{row}"] = "RUC"
            ws[f"C{row}"].font = header_font
            ...
            ws[f"A{row}"].font = header_font
            ws[f"A{row}"].border = no_border
            ws[f"B{row}"] = f': {notary_info["provincia"]}'
            ws[f"B{row}"].font = data_font
            ws[f"B{row}"].border = no_border
            ws[f"C{row}"] = "DESDE"
            ws[f"C{row}"].font = header_font
    ...
    def generate_word_report(self, desde, hasta):
        ...
        try:
            ...
            row3.cells[1].text = f': {notary_info["departamento"]}'
            ...
            row4.cells[1].text = f': {notary_info["provincia"]}'
