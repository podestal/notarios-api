...
def _smart_update_non_contentious_with_auto_discovery(
    template_id: int, kardex: str, idtipoacto: str
) -> dict:
    """
    ...
    try:
        ...
        try:
            ...
            print(
                f"DEBUG: Successfully downloaded current non-contentious document from R2 ({len(current_doc_content)} bytes)"
            )
            ...
            service = NonContentiousDocumentService()
            new_doc_response = service.generate_non_contentious_document(
                template_id, kardex, idtipoacto, "update"
            )

            if new_doc_response.status_code != 200:
                print(
                    f"DEBUG: Failed to generate updated non-contentious document: {new_doc_response.status_code}"
                )
                return {"error": "Failed to generate updated non-contentious document"}

            print(
                f"DEBUG: Successfully generated new non-contentious document ({len(new_doc_response.content)} bytes)"
            )
            ...
            merged_doc = _merge_documents_smart(
                current_doc_content, new_doc_response.content, kardex
            )
            ...
            s3.upload_fileobj(file_obj, os.environ.get("CLOUDFLARE_R2_BUCKET"), object_key)

            print(f"DEBUG: Successfully uploaded merged non-contentious document to R2")
            return {
                "status": "success",
                "message": f"Non-contentious document updated successfully for kardex: {kardex}",
            }
...
def _smart_update_with_auto_discovery(template_id: int, kardex: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            current_doc_content = response["Body"].read()
            print(
                f"DEBUG: Successfully downloaded current document from R2 ({len(current_doc_content)} bytes)"
            )

            # Step 2: Generate new document with updated data
            print(f"DEBUG: Generating new document with template_id: {template_id}")
            service = VehicleTransferDocumentService()
            new_doc_response = service.generate_vehicle_transfer_document(
                template_id, kardex, "update"
            )
            ...
            print(
                f"DEBUG: Successfully generated new document ({len(new_doc_response.content)} bytes)"
            )
...
def _merge_documents_smart(
    current_doc_content: bytes, new_doc_content: bytes, kardex: str
) -> bytes:
    """
    ...
    try:
        ...
        updated_doc = _smart_update_specific_fields(
            current_doc, new_doc, current_data, db_driven_fields
        )
        ...
        for j, row in enumerate(table.rows):
            for k, cell in enumerate(row.cells):
                for l, paragraph in enumerate(cell.paragraphs):
                    if paragraph.text.strip():
                        print(
                            f"DEBUG: Table {i}, Row {j}, Cell {k}, Paragraph {l}: '{paragraph.text[:200]}...'"
                        )
...
def _update_based_on_template_placeholders(paragraph, template_text, field_mapping):
    ...
    for placeholder, field_name in placeholder_mapping.items():
        if (
            field_name in field_mapping
            and field_mapping[field_name]
            and field_mapping[field_name].strip()
        ):
...
def _update_placeholder_in_runs(paragraph, placeholder, value):
    ...
    if not placeholder_found:
        print(
            f"DEBUG: Placeholder '{placeholder}' not found in individual runs, checking if it spans multiple runs"
        )
        paragraph_text = paragraph.text
        if placeholder in paragraph_text:
            print(
                f"DEBUG: Found '{placeholder}' spanning multiple runs in paragraph: '{paragraph_text[:100]}...'"
            )
                ...
                if cell_idx < len(new_table.rows[row_idx].cells):
                    ...
                    for paragraph_idx, paragraph in enumerate(cell.paragraphs):
                        if paragraph.text.strip():  # Only update non-empty paragraphs
                            print(
                                f"DEBUG: Table cell {row_idx},{cell_idx} paragraph {paragraph_idx}: '{paragraph.text[:100]}...'"
                            )
                            _smart_update_paragraph(
                                paragraph,
                                (
                                    new_cell.paragraphs[paragraph_idx]
                                    if paragraph_idx < len(new_cell.paragraphs)
                                    else paragraph
                                ),
                                field_mapping,
                            )
                            ...
                            print(
                                f"DEBUG: Table cell {row_idx},{cell_idx} paragraph {paragraph_idx} is empty, checking for placeholders"
                            )
                            _update_based_on_template_placeholders(
                                paragraph,
                                (
                                    new_cell.paragraphs[paragraph_idx].text
                                    if paragraph_idx < len(new_cell.paragraphs)
                                    else ""
                                ),
                                field_mapping,
    ...
    def open_template(self, request):
        ...
        if tipkar == 3:  # TRANSFERENCIAS VEHICULARES
            ...
            if mode == "open":
                ...
                return service.generate_non_contentious_document(
                    template_id, kardex, idtipoacto, action, mode
                )
        ...
        try:
            ...
            if tipkar == 3:  # TRANSFERENCIAS VEHICULARES
                ...
                if mode == "open":
                    ...
                    return service.generate_vehicle_transfer_document(
                        template_id, kardex, action, mode
                    )
            elif tipkar == 2:  # ASUNTOS NO CONTENCIOSOS
                print(f"DEBUG: Using NonContentiousDocumentService for tipkar {tipkar}")
