...
class ContratantesxactoViewSet(ModelViewSet):
    ...
    def by_kardex(self, request):
        ...
        renta_map = {
            c['idcontratante']: c
            for c in renta.values('idrenta', 'kardex', 'idcontratante', 'pregu1', 'pregu2', 'pregu3')
        }
...
class IngresoPoderesViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            last_record = models.IngresoPoderes.objects.filter(
                num_formu__isnull=False
            ).exclude(num_formu='').order_by('-id_poder').first()
